...
var findDuplicate = function(paths) {
    ...
    for(var path of paths){
        ...
        var dir = splited.shift()
        while(splited.length){
            var cur = splited.pop().split(".")
            var fileName = cur[0]
            var fileContent = cur[1]
            var usePath = dir+"/"+fileName+".txt"
            map.has(fileContent)?map.set(fileContent,map.get(fileContent).concat(usePath)):map.set(fileContent,[usePath])
        }
    }
    var ans = []
    for (var path of map.values()) {
