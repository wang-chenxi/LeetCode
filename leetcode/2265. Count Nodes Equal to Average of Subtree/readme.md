...
var averageOfSubtree = function(root) {
    var count = 0;
    var findAverage = function(root){
        if(!root) return [0,0]
        var [leftSum, leftCnt] = findAverage(root.left)
        var [rightSum,rightCnt] = findAverage(root.right)
        var sum =leftSum + rightSum + root.val
        var cnt = leftCnt + rightCnt + 1
        if (Math.floor(sum/cnt) == root.val){
            count++
            console.log(root.val,cnt,sum)
        } 
        return [sum,cnt]
    }
    findAverage(root)
